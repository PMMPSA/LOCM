...
class SkyblockForm{
		...
		$form = new SimpleForm(function(Player $player, ?int $data){
			...
			if($data == 0){
				if(EconomyAPI::getInstance()->myMoney($player) >= 500000){
					Server::getInstance()->getCommandMap()->dispatch(new ConsoleCommandSender(), "is increase ".$name." 2");
			...
			if($data == 1){
				if(Coin::getInstance()->getCoin($player) >= 50){
					Server::getInstance()->getCommandMap()->dispatch(new ConsoleCommandSender(), "is increase ".$name." 5");
					oin::getInstance()->reduceCoin($player, 50);
				}else{
					$player->sendMessage("§l§f• Không đủ tiền để nâng giới hạn đảo");
				}
			}
	...
	public function banForm(Player $player) :void
	{
		$form = new CustomForm(function(PLayer $player, ?array $data)
		{
			if(is_null($data)) $this->startForm($data);
			if(!isset($data[2])) $this->startForm($data);
			if($data[1] == true)
			{
		...
		$form = new CustomForm(function(Player $player, ?array $data)
			{
				...
				if($data[0] == true){
					$this->execCommand($player, "is delete ". $data[1]);
				}else {
