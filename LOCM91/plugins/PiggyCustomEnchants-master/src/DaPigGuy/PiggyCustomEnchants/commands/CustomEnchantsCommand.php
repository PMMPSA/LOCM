...
class CustomEnchantsCommand extends BaseCommand
{
    ...
    public function onRun(CommandSender $sender, string $aliasUsed, array $args): void
    {
        ...
        if ($sender instanceof Player && PiggyCustomEnchants::getInstance()->areFormsEnabled()) {
            $form = new SimpleForm(function (Player $player, ?int $data) use ($subcommands): void {
                if ($data !== null && isset($subcommands[$data])) {
                    PiggyCustomEnchants::getInstance()->getServer()->dispatchCommand($player, "ce " . $subcommands[$data]);
                }
            });
